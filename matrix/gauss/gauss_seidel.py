...
def main():
    filename = "test_seidel_nghich_dao.txt"
    try:
    ...
    if cols_a != m:
        print(f"Loi: Ma tran A phai vuong (dang doc duoc la {m} x {cols_a}), khong the ap dung PP lap Gauss-Seidel!")
        return
    ...
    X, n = gauss_seidel_iterate(A, B, eps)
    if X is None:
        return
